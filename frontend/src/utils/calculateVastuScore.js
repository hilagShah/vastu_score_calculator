/**
 * Client-side Vastu Score Calculation Helper
 * Ensures instant calculation fallback if backend API is unreachable or slow.
 */
export const calculateVastuScoreDetails = (inputs) => {
  const {
    mainEntrance = 'N',
    kitchen = 'SE',
    additionalKitchens = [],
  ...
  // Generate detailed defects lists
  const defects = [];
  if (hasBathroomInNE) {
    defects.push({
  ...
  return {
    _id: `local_${Date.now()}`,
    fullName: inputs.fullName || 'Valued User',
    email: inputs.email || '',
    phone: inputs.phone || '',
    inputs,
    totalScore: vastuScore,
    tier,
    breakdown,
    criticalDoshas,
    defects,
